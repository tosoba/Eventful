...
class AppRepository
@Inject
constructor(private val appContext: Context, private val rxLocation: RxLocation) : IAppRepository {

    override suspend fun usersLocation(): LocationResult =
        try {
            if (appContext.isLocationAvailable)
                rxLocation.currentLocation()?.let {
                    LocationResult.Found(it.latitude, it.longitude)
                }
                    ?: run { LocationResult.Error(FailedToFindLocationException) }
