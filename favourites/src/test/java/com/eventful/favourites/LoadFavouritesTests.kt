...
internal class LoadFavouritesTests : BaseFavouritesFlowProcessorTests() {
    ...
    fun loadFavouritesTest() =
        testScope.runBlockingTest {
            ...
            val getSavedEventsFlow =
                mockk<GetSavedEventsFlow> { every { this@mockk(any()) } returns flowOf(events) }
            val currentState =
                mockk<() -> FavouritesState> { every { this@mockk() } returns FavouritesState() }
    ...
    @Test
    @DisplayName(
        "On LoadFavourites - should call getSavedEventsFlow and emit an Update.Events for each event flow emission")
    fun loadFavouritesMultipleEmissionsTest() =
        testScope.runBlockingTest {
            val events1stEmission = mockedList(20) { event(it) }
            val events2ndEmission = mockedList(25) { event(it) }
            val getSavedEventsFlow =
                mockk<GetSavedEventsFlow> {
    ...
    fun loadingFavouritesOnStartTest() =
        testScope.runBlockingTest {
            val getSavedEventsFlow =
                mockk<GetSavedEventsFlow> {
            ...
            flowProcessor(getSavedEventsFlow = getSavedEventsFlow, loadFavouritesOnStart = true)
                .updates(intents = emptyFlow(), currentState = currentState)
                .toList()
    ...
    fun noLoadingFavouritesOnStartTest() =
        testScope.runBlockingTest {
            ...
            flowProcessor(getSavedEventsFlow = getSavedEventsFlow, loadFavouritesOnStart = false)
                .updates(intents = emptyFlow(), currentState = currentState)
                .toList()
