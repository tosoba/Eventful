...
internal class NewSearchTests : BaseFavouritesFlowProcessorTests() {
    ...
    @Test
    @DisplayName(
        "After NewSearch update is applied to state - should emit Update.Events with filtered events")
    fun newSearchLoadFavouritesTest() =
        testScope.runBlockingTest {
            ...
            val currentState =
                mockk<() -> FavouritesState> { every { this@mockk() } returns initialState }
