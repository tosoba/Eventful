...
fun Fragment.showAddEditAlarmDialog(
    ...
    AddEditAlarmDialog(requireContext(), mode, initialState).apply {
        setContentView(
            AddEditAlarmDialogBinding.inflate(layoutInflater)
                .apply {
                    alarmDialogTitle.text =
                        if (mode is AddEditAlarmDialogMode.Edit) {
...
class AddEditAlarmDialog(
    ...
    val time: MutableLiveData<AlarmTime> = MutableLiveData(initialState?.time ?: AlarmTime(mode))
    val date: MutableLiveData<AlarmDate> =
        MutableLiveData(initialState?.date ?: AlarmDate(mode.startDateCalendar))
