...
import com.eventful.core.android.*
...
fun <D, I> Fragment.infiniteItemListController(
    ...
    if (holder.data.isEmpty()) when (val status = holder.status) {
        is Initial -> imageBackgroundInfo {
        ...
        is LoadedSuccessfully -> if (emptyTextResource != null) imageBackgroundInfo {
            id("empty-background")
            imageRes(imageBackgroundResource)
            description(requireContext().getText(emptyTextResource(data)).toString())
        }
