...
data class ExternalLinks(
    ...
    @SerializedName("itunes") val iTunes: List<ExternalLink>?,
    val facebook: List<ExternalLink>?,
    val wiki: List<ExternalLink>?,
    val instagram: List<ExternalLink>?,
    @SerializedName("musicbrainz") val musicBrainz: List<ExternalId>?,
    @SerializedName("homepage") val homePage: List<ExternalLink>?
) {
    val links: List<ILink>
        get() =
            listOf(
                ...
                .map { ILink.with(it.second!!, LinkType.fromString(it.first)!!) }
}
