...
internal class LoadMoreResultsTests : BaseNearbyFlowProcessorTests() {
    ...
    private fun updatesFlow(
        ...
        locationStateProvider: LocationStateProvider = mockk {
            every { locationStates } returns
                flowOf(LocationState(LatLng(10.0, 10.0), LocationStatus.Initial))
        },
    ...
    fun loadMoreResultsLoadingTest() =
                    ...
                    currentState =
                        mockk {
                            every { this@mockk() } returns
                                NearbyState(
                                    PagedDataList(status = Loading, data = relaxedMockedList(20)))
                        })
                .launchIn(this)
    ...
    fun loadMoreResultsWhenEmptyTest() =
        testScope.runBlockingTest {
            val getNearbyEvents = mockk<GetNearbyEvents>(relaxed = true)
            ...
            updatesFlow(
                    ...
                    currentState =
                        mockk {
                            every { this@mockk() } returns
                                NearbyState(PagedDataList(data = emptyList()))
                        })
                .launchIn(this)
    ...
    fun loadMoreResultsWhenEmptyTestWhenLimitHit() =
        testScope.runBlockingTest {
            val getNearbyEvents = mockk<GetNearbyEvents>(relaxed = true)
                    ...
                    currentState =
                        mockk {
                            every { this@mockk() } returns
                                NearbyState(
                                    PagedDataList(
                                        data = relaxedMockedList(20), offset = 2, limit = 1))
                        })
                .launchIn(this)
