...
sealed class SearchStateUpdate : StateUpdate<SearchState> {
    data class ToggleEventSelection(override val item: Event) :
        SearchStateUpdate(), ToggleItemSelectionUpdate<SearchState, Event, String> {
        override fun Event.id(): String = id
    }

    object ClearSelection : SearchStateUpdate(), ClearSelectionUpdate<SearchState, Event>
    ...
    sealed class Events : SearchStateUpdate() {
        ...
        data class Loaded(val resource: Resource<PagedResult<IEvent>>, val newSearch: Boolean) :
            Events() {
            override fun invoke(state: SearchState): SearchState =
                state.run {
                    when (resource) {
        ...
        ) : SearchStateUpdate(), ItemSelectionConfirmedUpdate<SearchState, Event>
    }
}
